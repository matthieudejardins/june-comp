# task_plan.md — Pilote « Madeleine » (BLAST)
...
> Le graphe PROJET `madeleine-pilote` (77 nœuds) reste, lui, indexé dans le dashboard.
...
- [x] Skill `madeleine-session/SKILL.md` complet (rôle, lecture corpus fermé, 4 phases, 6 invariants verbatim, escalade L1/L3/L5, fin de séance → journal)
...
- [x] Skill installé dans Hermes (`~/.hermes/skills/madeleine-session`, enabled) + **séance live OK** :
      `hermes -z … --skills madeleine-session --yolo` → dit « Geneviève », oriente (vendredi 26 juin),
