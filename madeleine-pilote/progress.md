...
**Fait :**
- Arborescence projet `madeleine-pilote/` créée (couches A.N.T. : `architecture/`, `tools/`, `.tmp/`).
...
- Phase L **réécrite** : réutiliser claude-os, ne PAS installer Ollama. Ajouter un graphe
  patient `madeleine-mme-durand` au registre (séparé de `acredity`).
...
## 2026-06-26 (suite 2) — Intégration UX : graphe Madeleine dans le dashboard claude-os
**Demande utilisateur :** indexer un graphe Madeleine dans le dashboard, conjointement à Acredity,
...
- Graphe construit + enregistré via l'endpoint d'ingestion du dashboard (`POST /__graphify_ingest`,
  même chemin que l'UI « Add a project ») : projet **`madeleine-pilote`**, nom **« Madeleine »**,
  **77 nœuds · 59 liens · 18 clusters**, god nodes = CLAUDE.md, task_plan, BRIEF, findings, exercices, SOP.
  Fichier : `claude-os/src/data/graphs/madeleine-pilote.json` ; entrée ajoutée à `index.json`
  (à côté de `acredity`, couleur `#60a5fa`, description métier).
- **`/codegraph`** : Madeleine apparaît automatiquement dans la galerie avec visualisation 3D
  (la page lit `index.json` / `/__graphify_list`). Item de menu « Knowledge Graph » déjà présent.
- **`/` (home)** : section 2.5 bascule de `acredity.json` → `madeleine-pilote.json`
  (`src/routes/index.tsx` : import L31, label « Madeleine », accent `#60a5fa`, légende réécrite).
  Les compteurs nœuds/liens/clusters se recalculent depuis l'import.

**Tests :** `/` et `/codegraph` → HTTP 200 ; `/__graphify_list` renvoie acredity (1350) + madeleine-pilote (77).

**Note produits :** Acredity et Madeleine restent **deux graphes séparés** dans le registre
(co-listés, jamais fusionnés). Graphe ingéré = projet `madeleine-pilote` complet ; un sous-graphe
...
- `skill/madeleine-session/SKILL.md` : skill complet (frontmatter, lecture du corpus fermé,
...
- Skill copié dans `~/.hermes/skills/madeleine-session` (`hermes skills list` = enabled).
- `cd madeleine-pilote && hermes -z "<lance Phase 1+2 pour mme-durand>" --skills madeleine-session --yolo`
...
Skill resynchronisé dans `~/.hermes/skills/madeleine-session`. `make test` = safety 5/5 + SRT/SFA ✅.
...
- Cause : le skill utilisait des chemins RELATIFS ; le gateway Hermes ne tourne pas depuis le
  ...
  (BASE=`/Users/matthieu/Madeleine/madeleine-pilote`) + resync Hermes.
- **Vérifié** : `hermes -z … --skills madeleine-session` lancé **depuis ~** lit bien
  `exercices.md` (« Récupération Espacée… ») et le carnet (« je regarde mon carnet de mémoire »). ✅

**#2 Cron (initiation proactive) — ACTIF :**
- `hermes cron create "0 9 */2 * *" … --name madeleine-mme-durand --skill madeleine-session
