# SOP — Réutiliser le runtime `Acredity/claude-os` pour le pilote Cognitive Companion

> Layer 1 (Architect). Golden Rule : si la logique change, mettre à jour cette SOP **avant** le code.
> Objectif : ne PAS réinstaller Hermes/Ollama ; brancher Cognitive Companion sur l'OS déjà câblé.
...
3. **Construire le graphe patient** :
   ```bash
   graphify /Users/matthieu/Cognitive Companion/cognitive-companion/patients/mme-durand
...
4. **Enregistrer dans le registre partagé** (entrée DISTINCTE de `acredity`) :
   ...
   `{ "id": "cognitive-companion-mme-durand", "name": "Cognitive Companion — Mme Durand", "path": "…/patients/mme-durand", "graphPath": "…/graph.json", … }`.
...
- **Séparation des produits** : ne jamais fusionner le graphe `acredity` et le graphe Cognitive Companion.
- **Invariant n°6** : stack cloud OK pour proto fictif uniquement ; tracer le repatriement local
  requis avant tout patient réel.
- **Skill `companion-session`** : installer comme skill Hermes en phase A ; system prompt = 6
