# CLAUDE.md — Constitution du projet « Cognitive Companion » (pilote)
...
Construire un **pilote démontrable** de « Cognitive Companion » : un agent qui, à heure fixe, ouvre
...
|---|---|---|
| Mémoire | **Graphify** installé (`graphify` sur PATH ; `graphifyy` PyPI) + registre partagé `claude-os/src/data/graphs/index.json` | Graphe patient interrogeable. On y **ajoute une entrée `cognitive-companion-mme-durand`** (séparée du graphe `acredity`). |
...
- 2026-06-26 — Graphe Cognitive Companion (projet `cognitive-companion`, 77 nœuds) ingéré dans le dashboard claude-os via `/__graphify_ingest`. Co-listé avec Acredity sur `/codegraph` ; home `/` bascule sur ce graphe (`src/routes/index.tsx`). Fichiers modifiés HORS de ce projet (dans `claude-os`) — voir `progress.md`.
