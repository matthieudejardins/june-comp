# Cognitive Companion: a cognitive-health startup for the elderly, on the Claude Code OS

**June Comp entry.** A new feature for the Claude Code Agentic OS: it turns the OS into
**Cognitive Companion**, a voice that runs a daily speech-therapy session for someone with early
...
| # | Scenario | What Cognitive Companion does | Technique |
|---|---|---|---|
...
## What's in here
- `cognitive-companion/` — the build: the `companion-session` skill, the patient file
...
- `integration/` — the files that plug into Claude Code OS: the voice-lab server with the
  Cognitive Companion persona, a small OpenAI proxy, and the two graph files.
...
1. Drop `integration/voice-lab-server.ts` over `claude-os/voice-lab/server.ts` (it carries
   Cognitive Companion's persona + the patient context).
2. Put `OPENAI_API_KEY=...` in `claude-os/.env`.
3. Run the dashboard and voice-lab, open the Hermes view, hit "Talk to Hermes", and say:
   *"Bonjour Cognitive Companion, on fait notre conversation du matin ?"*
   See `integration/INTEGRATION.md` for the full wiring.

## Demo
60-second Loom: https://www.loom.com/share/9ddab64c27e8400bbcad08f73ce7dcdd
