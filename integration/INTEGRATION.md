# Wiring Cognitive Companion into the Claude Code OS
...
- `voice-lab-server.ts` → copy over `claude-os/voice-lab/server.ts`. It carries the
  Cognitive Companion persona and Geneviève's context directly in the Realtime session `instructions`,
  so the orb voice plays Cognitive Companion (in French), runs the 4-phase session, leads to the
...
- `graphs/cognitive-companion.json` and `graphs/patient.json` → copy into
  `claude-os/src/data/graphs/` and add entries to `index.json` so they show in the Knowledge
  Graph view. `patient.json` is the 41-node patient memory the agent reasons over.

## The skill
Install `../cognitive-companion/skill/companion-session` as a Hermes skill (or point the
...
```
Open the Hermes view, "Talk to Hermes", say "Bonjour Cognitive Companion, on fait notre conversation
du matin ?". Start a fresh call after any change (the persona loads at call start).

## Not included (privacy)
The dashboard tab `src/routes/cognitive-companion.tsx` and the IBM personality assets are left out of
